...
def datagen(
    ...
    for i, mel in enumerate(mel_chunks):
        idx = (i+delay_frame)%len(face_frames)
        face = face_frames[idx]
        mel_batch.append(mel)
        face_batch.append(face)

        if len(face_batch) >= batch_size:
            face_batch, mel_batch = np.asarray(face_batch), np.asarray(mel_batch)
            # face_batch = np.concatenate([face_batch, face_batch], axis=-1) / 255.  3通道变为6通道
            face_masked = face_batch.copy()
            face_masked[:,384//2:] = 0 
            face_batch = np.concatenate((face_masked, face_batch), axis=3) / 255.
            mel_batch = np.reshape(mel_batch, [len(mel_batch), mel_batch.shape[1], mel_batch.shape[2], 1])
            yield mel_batch, face_batch
            mel_batch, face_batch = [], []
